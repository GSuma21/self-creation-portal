  ...
  ngOnInit() {
    ...
    this.subscription.add(
      this.libProjectService.currentProjectMetaData.subscribe((data) => {
        ...
        // });
        this.lastReviewed =
          this.mode === 'review' || this.mode === 'reviewerView'
            ? this.libProjectService.projectData.last_reviewed_on
            : '';
  ...
  setConfig() {
    this.subscription.add(
      this.libProjectService.setConfig().subscribe((res: any) => {
        ...
        this.libProjectService.projectConfig = res.result.resource.find(
          (res: any) =>
            res.resource_type === this.configuration.permissionCoFig.PROJECTS
        );
      })
    );
  }
  ...
  getProjectdata() {
    ...
    this.subscription.add(
      this.formService.getForm(SOLUTION_LIST).subscribe((form) => {
        projectData = form?.result?.data?.fields?.controls.find(
          (item: any) => item.title === 'PROJECT'
        );
      })
    );
    this.formService
      .getFormWithEntities('PROJECT_DETAILS')
      .then((result) => {
        this.subscription.add(
